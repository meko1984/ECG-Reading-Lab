...
const outsidePositions: Record<ElectrolyteId, Array<[number, number]>> = {
  ...
  k: [[102, 330], [185, 360], [274, 326], [372, 360], [480, 329], [586, 358], [666, 328]],
  cl: [[455, 76], [565, 94], [652, 62], [710, 100], [394, 54], [600, 48], [494, 105]],
  ca: [[305, 70], [370, 102], [430, 70], [510, 105], [583, 72], [650, 110], [338, 50]],
  mg: [[690, 66], [720, 92], [668, 112], [628, 82], [704, 44], [622, 48], [742, 68]],
};

const insidePositions: Record<ElectrolyteId, Array<[number, number]>> = {
  na: [[152, 298], [236, 340], [535, 304], [622, 342], [334, 374], [450, 310], [700, 370]],
  k: [[78, 286], [135, 352], [220, 296], [310, 350], [415, 292], [526, 354], [650, 292]],
  cl: [[250, 380], [570, 386], [710, 305], [95, 385], [390, 340], [480, 390], [610, 310]],
  ca: [[340, 306], [585, 320], [700, 350], [175, 380], [440, 370], [282, 285], [500, 330]],
...
function CellMembrane({ phase, settings }: { phase: ActionPhase; settings: ElectrolyteSettings }) {
  ...
  return (
    <svg className={styles.cellSvg} viewBox="0 0 780 430" role="img" aria-label={`${PHASE_INFO[phase].title}。${PHASE_INFO[phase].short}`}>
      ...
      {ELECTROLYTES.flatMap((ion) => insidePositions[ion].slice(0, ion === 'k' ? 7 : Math.max(2, 5 - Math.round(settings[ion] * 2))).map(([x, y], index) => <IonDot key={`i-${ion}-${index}`} ion={ion} x={x} y={y} />))}
      {([['na', 180], ['ca', 355], ['k', 530], ['cl', 665]] as Array<[ElectrolyteId, number]>).map(([ion, x]) => <g key={ion} style={{ color: ELECTROLYTE_META[ion].color }}><rect x={x - 25} y="168" width="50" height="84" rx="20" className={styles.channel} /><line x1={x - 12} x2={x + 12} y1="210" y2={phase === 4 ? '210' : '199'} className={styles.channelGate} /><text x={x} y="221" textAnchor="middle" className={styles.channelLabel}>{ELECTROLYTE_META[ion].symbol}</text></g>)}
      ...
      <g transform="translate(700 196)"><circle r="18" fill={ELECTROLYTE_META.mg.color} /><text y="1" className={styles.ionText}>Mg</text><text x="0" y="42" textAnchor="middle" className={styles.channelLabel} fill={ELECTROLYTE_META.mg.color}>調節</text></g>
    </svg>
  );
}
    ...
    <div className={styles.lab}>
      ...
      <div className={styles.visualGrid}>
        <section className={`${styles.panel} ${styles.cellPanel}`} aria-labelledby="cell-heading">
          ...
          <CellMembrane phase={phase} settings={settings} />
